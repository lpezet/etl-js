		...
		try {
			...
			if ( pConfig['content'] ) {
				...
				.then( function( pData) {
					...
					logger.error('[%s] Error executing ECL.', pParent, pError);
...
ModClass.prototype.handle = function( pParent, pConfig, pExecutor, pActivityContext, pGlobalContext ) {
	...
	return new Promise( function( resolve, reject ) {
		...
		try {
			...
			for (var i in pConfig) {
				var oKey = i;
				oKey = oKey.indexOf("{{") < 0 ? oKey: that._evaluate( oKey, oContext );
				
				var oECLConfig = that._read_config( pConfig[i] );
				oPromises.push( that._wrap_run( pParent, oKey, oECLConfig, pExecutor, oContext ) );
			}
