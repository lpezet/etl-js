...
ModClass.prototype._chain_handler = function( pStepId, pKey, pStep, pHandler, pResult, pContext ) {
	var that = this;
	return function( pCurrentStepData ) { 
		try {
			return pHandler.handle( pStepId, pStep, that.mExecutor, pCurrentStepData, pResult, pContext );
		} catch( e ) {
...
ModClass.prototype._wrap_process = function( pStepIndex, pTotalSteps, pStepId, pStep, pResult, pContext ) {
	var that = this;
	return function( pData ) { 
		that.mLogger.info('[%s] Executing activity (%s/%s)...', pStepId, pStepIndex, pTotalSteps );
		if ( pData['_exit'] ) {
			//TODO: log exit behavior here. Use _exit_from to log which section triggered exit.
			return Promise.resolve( pData ); //TODO: resolve?
		} else {
			try {
				return that._process( pStepIndex, pTotalSteps, pStepId, pStep, pData, pResult, pContext );
			} catch (e) {
...
ModClass.prototype._process = function( pStepIndex, pTotalSteps, pStepId, pStep, pPreviousStepData, pResult, pContext ) {
	var that = this;
		...
		try {
			...
			for (var i in pStep ) {
				...
				if ( ! oMod ) {
					...
					oProcesses.push( that._chain_handler( pStepId, i, pStep[i], oMod, pResult, pContext ) );
				}
			}
			Promises.seq( oProcesses, pPreviousStepData ).then( function( pData ) {
				that.mLogger.info('[%s] Activity completed (%s/%s).', pStepId, pStepIndex, pTotalSteps);
				/*
				...
				*/
				...
				//TODO: no need then:
				pResult[ pStepId ] = pData;
				//TODO: replace with:
				//resolve( pPreviousStepData );
				that.emit('activityDone', pStepId, null, pData, pStepIndex, pTotalSteps );
				resolve( pData );
			},  function( pError ) {
...
ModClass.prototype.process = function( pConfig, pParameters ) {
	...
	return new Promise( function( resolve, reject ) {
		...
		try {
			...
			const oTotalSteps = oETLSteps.length;
			var oStepProcesses = [];
			var oResult = { 
				etl: {
					exit: false
			...
			for (var i =0; i < oETLSteps.length; i++) {
				var oStepId = oETLSteps[i];
				var oStep = pConfig[oStepId];
				if ( ! oStep ) {
					//TODO
					that.mLogger.warn('No configuration for activity [%s] (%s/%s). Skipping.', oStepId, i+1, oTotalSteps);
				} else {
					that.mLogger.debug('Encountered activity [%s]...', oStepId);
					oStepProcesses.push( that._wrap_process( i+1, oTotalSteps, oStepId, oStep, oResult, oContext ) );
				}
			}
