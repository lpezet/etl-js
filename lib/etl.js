...
ModClass.prototype._wrap_process = function( pStepIndex, pTotalSteps, pStepId, pStep, pContext ) {
	var that = this;
	return function( pData ) { 
		...
		if ( pData['_exit'] ) {
			...
			try {
				return that._process( pStepIndex, pTotalSteps, pStepId, pStep, pData, pContext );
			} catch (e) {
...
ModClass.prototype._process = function( pStepIndex, pTotalSteps, pStepId, pStep, pPreviousStepData, pContext ) {
	var that = this;
		...
		try {
			...
			for (var i =0; i < oETLSteps.length; i++) {
				...
				if ( ! oStep ) {
					...
					oStepProcesses.push( that._wrap_process( i+1, oTotalSteps, oStepId, oStep, oData ) );
				}
			}
