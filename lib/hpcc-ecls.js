					...
					case "output": 
						if ( pConfig[i] ) {
							...
							if ( oOutput.indexOf("{{") >= 0 ) {
								...
								oOutput = oOutputs[ oTemplateIndex ];
							}
...
ModClass.prototype.handle = function( pParent, pConfig, pExecutor, pContext ) { //pCurrentActivityResult, pGlobalResult, pContext ) {
	...
	return new Promise( function( resolve, reject ) {
		...
		try {
			...
			var oResolvedConfig = that.mTemplateEngine.evaluateObject( pConfig, pContext );
			//console.log('###############################################');
			...
			Object.keys(oResolvedConfig).forEach( k => {
				...
				oPromises.push( that._wrap_run( pParent, k, oECLConfig, pExecutor, pContext ) );
			});
