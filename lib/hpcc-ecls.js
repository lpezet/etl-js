...
ModClass.prototype._run = function( pPreviousData, pParent, pKey, pConfig, pExecutor, pContext ) {
	...
	return new Promise( function( resolve, reject) {
		try {
			...
			if ( (pConfig['file'] && pConfig['file'].startsWith('file://')) || pConfig['content'] ) {
				var oContent = pConfig['content'];
				if ( pConfig['file'] ) {
					var oFileURL = pConfig['file'];
					oFileURL = oFileURL.indexOf("{{") < 0 ? oFileURL: that._evaluate( oFileURL, pContext );
					
					if ( oFileURL.startsWith('file://') ) {
						var oPath = oFileURL.substring(7);
						//console.log('file path=' + oPath);
						oPath = path.resolve( process.cwd(), oPath );
						//console.log('file RESOLVED path=' + oPath);
						oContent = fs.readFileSync(oPath, 'utf8');
					}
				}
				...
				// if scheme is file://, e.g.  file:///Users/lucpezet/Work/luc/etl-projects/nppes/index.html
...
ModClass.prototype.handle = function( pParent, pConfig, pExecutor, pContext ) { //pCurrentActivityResult, pGlobalResult, pContext ) {
	...
	return new Promise( function( resolve, reject ) {
		...
		try {
			...
			for (var i in pConfig) {
				var oKey = i;
				oKey = oKey.indexOf("{{") < 0 ? oKey: that._evaluate( oKey, pContext );
				
				var oECLConfig = that._read_config( pParent, oKey, pConfig[i] );
				oPromises.push( that._wrap_run( pParent, oKey, oECLConfig, pExecutor, pContext ) );
			}
