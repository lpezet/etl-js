...
export default class HPCCFrictionlessMod extends AbstractMod<any> {
  ...
  _doFrictionless(
    ...
    if (source === null || source === undefined) {
      return [
        () => Promise.reject(new Error("Must specify property 'source'."))
      ];
    }
    if (source.indexOf("{{") >= 0) {
    ...
    if (
      ...
      return [
        () => Promise.reject(new Error("Must specify property 'resources'."))
      ];
    }
