...
ModClass.prototype._spray_fixed = function( pParent, pKey, pSprayConfig, pExecutor, pContext, pTemplateIndex ) {
	return function() {
...
ModClass.prototype._spray_xml = function( pParent, pKey, pSprayConfig, pExecutor, pContext, pTemplateIndex ) {
	return function( pPreviousData ) {
		return new Promise( function( resolve, reject ) {
			var data = { error: 'Spray fixed is not yet supported.', result: null, message: null, exit: false, pass: true, _stdout: null, _stderr: null  };
...
ModClass.prototype._spray_delimited = function( pParent, pKey, pSprayConfig, pExecutor, pContext, pTemplateIndex ) {
					...
					switch ( k ) {
						...
						case "sourcepath":
							if ( pSprayConfig[k] ) {
								...
								var oSrcPaths = oSrcPath.indexOf("{{") < 0 ? [oSrcPath]: that._evaluate( oSrcPath, pContext );
	...
	return new Promise( function( resolve, reject ) {
		...
		try {
			...
			for (var i in pConfig) {
				...
				var oLogicalFileNames = oLogicalFileName.indexOf("{{") < 0 ? [oLogicalFileName]: that._evaluate( oLogicalFileName, pContext );
				oLogicalFileNames.forEach( function(e, j ) {
					var oSprayConfig = that._read_config( pParent, i, pConfig[i] );
					if ( ! oSprayConfig.destinationlogicalname ) oSprayConfig.destinationlogicalname = e;
					...
					if ( ! oSprayConfig.format ) {
						...
						switch ( oSprayConfig.format ) {
							case "delimited":
							case "csv":
								oPromises.push( that._spray_delimited( pParent, e, oSprayConfig, pExecutor, pContext, j ) );
								break;
							case "fixed":
								oPromises.push( that._spray_fixed( pParent, e, oSprayConfig, pExecutor, pContext, j) );
								break;
							case "xml":
								oPromises.push( that._spray_xml( pParent, e, oSprayConfig, pExecutor, pContext, j ) );
								break;
							default:
