...
ModClass = function( pETL, pSettings ) {
	...
	var that = this;
	if( pETL ) pETL.mod( 'mysqlimports', this, function( pSettings, pLogger ) {
...
ModClass.prototype._read_options = function( pParent, pKey, pConfig, pExecutor, pContext, pTemplateIndex ) {
	var oOptions = {
		bind_address: null,
		columns: null,
		compress: null,
		...
		try {
			...
			for (var i in pConfig) {
				...
				oKeys.forEach( function( e, j ) {
					var oOptions = that._read_options( pParent, e, oConfig, pExecutor, pContext, j );
					oPromises.push( that._wrap_run( pParent, e, oOptions, pExecutor, pContext, j ) );
				});
