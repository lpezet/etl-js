...
ModClass.prototype._spray_fixed = function( pParent, pSprayConfig, pExecutor ) {
	return function() {
		return new Promise( function( resolve, reject ) {
			...
			reject( 'Spray fixed is not yet supported.' );
		});
	}
}
		...
		try {
			...
			for (var i in pConfig) {
				...
				if ( ! oSprayConfig.format ) {
					oPromises.push( that._spray_error( pParent, i ) );
				} else {
					switch ( oSprayConfig.format ) {
						case "delimited":
						case "csv":
							oPromises.push( that._spray_delimited( pParent, oSprayConfig, pExecutor ) );
							break;
						case "fixed":
							oPromises.push( that._spray_fixed( pParent, oSprayConfig, pExecutor ) );
							break;
						case "xml":
							oPromises.push( that._spray_xml( pParent, oSprayConfig, pExecutor ) );
							break;
						default:
