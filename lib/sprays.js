...
ModClass = function( pETL, pSettings ) {
	...
	var that = this;
	if( pETL ) pETL.mod( 'sprays', this, function( pSettings, pLogger ) {
...
ModClass.prototype.handle = function( pParent, pConfig, pExecutor, pActivityContext, pGlobalContext ) {
	...
	return new Promise( function( resolve, reject ) {
		...
		var oContext = { local: pActivityContext, global: pGlobalContext };
		try {
			var oData = { 'sprays' : {} };
			var oPromises = [];
			...
			for (var i in pConfig) {
				...
				oLogicalFileName = oLogicalFileName.indexOf("{{") < 0 ? oLogicalFileName: that._evaluate( oLogicalFileName, oContext );
				...
				if ( ! oSprayConfig.format ) {
					...
					switch ( oSprayConfig.format ) {
						case "delimited":
						case "csv":
							oPromises.push( that._spray_delimited( pParent, oLogicalFileName, oSprayConfig, pExecutor, oContext ) );
							break;
						case "fixed":
							oPromises.push( that._spray_fixed( pParent, oLogicalFileName, oSprayConfig, pExecutor, oContext) );
							break;
						case "xml":
							oPromises.push( that._spray_xml( pParent, oLogicalFileName, oSprayConfig, pExecutor, oContext ) );
							break;
						default:
